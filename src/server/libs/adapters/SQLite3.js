...
module.exports = class SQLite3 {
   ...
   */
   get currentPromise() {
      if (this._currentPromise == undefined) {
         this._currentPromise = null;
      }
      return (this._currentPromise);
   }

   set currentPromise(promiseSet) {
      this._currentPromise = promiseSet;
   }
   /**
   ...
   onSTDOUT(data) {
      if (this._firstCapture == false) {
         ...
         if (this.currentPromise != null) {
            var results = this.collateResults(data.toString(), this.currentPromise.schema);
            this.currentPromise.resolve.call(this, results);
            this._busy = false;
            this.processQueue();
         }
      } else {
   ...
   */
   processQueue() {
      ...
      if (this.requestQueue.length > 0) {
         this._busy = true;
         this.currentPromise = this.requestQueue.pop(); //remove from end (oldest)
         this._sqlite3.stdin.write(this.currentPromise.sql+"\n");
      }
   }
   ...
   */
   query(SQL, schemaArray=null) {
      var promise = new Promise((resolve, reject) => {
         if (this._busy == false) {
            this.currentPromise = new Object();
            this.currentPromise.schema = schemaArray;
            this.currentPromise.sql = SQL;
            this.currentPromise.resolve = resolve;
            this.currentPromise.reject = reject;
            ...
            this.requestQueue.unshift(queuedPromise); //add newest to beginning
         }
         this._busy = true;
      });
      return (promise);
