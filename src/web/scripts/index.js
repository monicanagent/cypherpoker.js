               ...
               .then((promise) => {
                  if (promise.target.response != null) {
                     ...
                     cypherpoker
                        ...
                        .then((result) => {
                           console.log(
                              "CypherPoker.JS instance fully started and connected."
                           );
                           resolve(true);
                        })
                        .catch((err) => {
                     ...
                     alert(
                        `Settings data (${_settingsURL}) not loaded or parsed.`
                     );
                     throw new Error(
                        `Settings data (${_settingsURL}) not loaded or parsed.`
                     );
...
async function onLoadJavaScript(event) {
   ...
   console.log(
      `"${loadedURL}" loaded at ${loadedTimeStamp.getSeconds()}s ${loadedTimeStamp.getMilliseconds()}ms`
   );
   if (_require.length > 0) {
      loadJavaScript(_require[0].url);
   } else {
      console.log(
         `All scripts loaded in ${loadedTimeStamp.getSeconds()}s ${loadedTimeStamp.getMilliseconds()}ms`
      );
   }
   ...
   switch (request.command) {
      default:
         ...
         response.data.message =
            'Unrecognized IPC request command "' + request.command + '"';
         break;
   }
