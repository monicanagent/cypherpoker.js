...
class CypherPokerUI {
    ...
    * @property {String} gameUISelectors.callButton="#callButton" The game's check/call button.
    ...
    * @property {String} gameUISelectors.timeoutAmount="#timeoutAmount" The game's timeout counter container element.
    * @property {String} gameUISelectors.dealerPuck="#dealerPuck" Puck is a poker term name for button.
   ...
   onLobbyCullTick(ui) {
      for (
         var count = 0;
         count < ui.cypherpoker.announcedTables.length;
         count++
      ) {
         var currentTable = ui.cypherpoker.announcedTables[count];
         var tableInfo = currentTable.tableInfo;
         var announceTimeout =
            Number(ui.cypherpoker.settings.lobbyDefaults.announceTimeout) *
         ...
         if (delta >= announceTimeout) {
            try {
               tableInfo.buttonElement.removeEventListener(
                  "click",
                  ui.onJoinTableButtonClick
               );
   ...
   set cypherpoker(cpSet) {
      if (this._cypherpoker == undefined || this._cypherpoker == null) {
         ...
         if (this._cypherpoker.apiConnected) {
            ...
            this._cypherpoker.addEventListener(
               "start",
               (event) => {
                  if (this._cypherpoker.apiConnected) {
                     var msg = "Connected to API services provider.<br/>";
                     ...
                     msg =
                        "Connection to API services provider not established.<br/>";
                  }
                  if (this._cypherpoker.p2pConnected) {
                     msg += "Connected to peer-to-peer network.";
                  } else {
                     msg +=
                        "Connection to peer-to-peer network not established.";
                  }
                  this.showDialog(msg);
                  this.hideDialog(5000);
                  this.updateAccountsUI();
   ...
   removeClone(sourceElement) {
      if (
         sourceElement["_clone"] == undefined ||
   ...
   addGameUIHandlers(gameElement, gameRef) {
      ...
      var callButton = gameElement.querySelector(
         this.gameUISelectors.callButton
      );
      var foldButton = gameElement.querySelector(
         this.gameUISelectors.foldButton
      );
      var newHandButton = gameElement.querySelector(
         this.gameUISelectors.newHandButton
      );
      //element-scoped event listeners
      betButton.addEventListener("click", this.onBetButtonClick);
      callButton.addEventListener("click", this.onCallButtonClick);
      ...
      gameRef.addEventListener("gamedealmsg", this.onGameDealMsg, this);
   ...
   startTimeoutTimer(timeoutElement, timeoutAmount, game) {
      ...
      timeoutElement._timeoutID = setTimeout(
         this.onTimeoutTimerTick,
         1000,
         timeoutElement,
         timeoutAmount,
         game,
   ...
   onTimeoutTimerTick(timeoutElement, timeoutAmount, game, ui) {
      ...
      var secondsElapsed = Math.floor(
         (now.valueOf() - timeoutElement._startTime) / 1000
      );
      ...
      if (secondsRemaining > 0) {
         ...
         if (secondsRemaining <= 5) {
            timeoutElement.innerHTML =
               '<span id="timeoutCritical">' + timeStr + "</span>";
         } else {
            timeoutElement.innerHTML =
               '<span id="timeout">' + timeStr + "</span>";
         }
         timeoutElement._timeoutID = setTimeout(
            ui.onTimeoutTimerTick,
            1000,
            timeoutElement,
            timeoutAmount,
            game,
         ...
         ui.showDialog(
            "A player has timed out. Awaiting confirmation from contract..."
         );
      }
   }
   ...
   addCypherPokerHandlers() {
      ...
      this.cypherpoker.addEventListener(
         "tablenew",
         this.onNewTableAnnouncement,
         this
      );
      ...
      switch (selection) {
         case "newWindow":
            if (isDesktop()) {
               ...
               try {
                  var windowName =
                     "CypherPoker.JS-" + String(Math.random()).split("0.")[1];
         ...
         case "manageConnections":
            var manageElement =
               this.getTemplateByName("connectivityManage").elements[0];
            ...
            var serverTableElement = manageElement.querySelector(
               "#serverTableDivider"
            );
            if (isDesktop()) {
         ...
         case "manageAccount":
            if (this.selectedAccount == null) {
               if (this.cypherpoker.apiConnected == false) {
                  var msg =
                     "You must connect to an API services provider in order to manage accounts";
               } else {
                  msg =
                     'You must login to the account ("USE ACCOUNT") you want to manage.';
               }
               this.showDialog(msg);
               this.hideDialog(5000);
               return;
            ...
            var manageElement =
               this.getTemplateByName("accountManage").elements[0];
            var loginElement =
               this.getTemplateByName("accountLogin").elements[0];
            ...
            manageElement.querySelector("#accountBalance").innerHTML =
               "updating...";
            loginElement.querySelector("#accountBalance").innerHTML =
               "updating...";
            this.selectedAccount
               .update()
               .then((done) => {
                  var satoshiAmount = this.selectedAccount.balance.toString(10);
                  var btcAmount = this.convertDenom(
                     satoshiAmount,
                     "satoshi",
                     "bitcoin"
                  );
         ...
         case "gameLobby":
            ...
            if (this.lobbyActive) {
               var confirmElement =
                  this.getTemplateByName("lobbyConfirm").elements[0];
            ...
            if (this.selectedAccount == null) {
               this.showDialog(
                  'You must login to an account ("USE ACCOUNT") with a balance in order to access the lobby.'
               );
               this.hideDialog(5000);
            } else {
               //lobby is not active or game has ended
               this.cypherpoker.removeAllGames(true); //remove any completed games
               var lobbyContainer = document.querySelector(
                  ui.UISelectors.lobby
               );
         ...
         case "logOut":
            var confirmElement =
               this.getTemplateByName("logOutConfirm").elements[0];
            if (this.lobbyActive) {
               this.hide(confirmElement.querySelector("#gameActive"));
         ...
         case "about":
            if (isDesktop()) {
               ...
               for (var db in dbInfoObj) {
                  ...
                  if (typeof currentDBInfo == "object") {
                     databaseInfo +=
                        currentDBInfo.version +
                        " (" +
                        currentDBInfo.dbSizeMB +
                        " MB of " +
                        currentDBInfo.dbMaxMB +
                        " MB)<br/>";
                  }
               }
            } else {
            ...
            helpElement.innerHTML = helpElement.innerHTML
               .split("%appTitle%")
               .join(appTitle);
            helpElement.innerHTML = helpElement.innerHTML
               .split("%runtime%")
               .join(runtime);
            helpElement.innerHTML = helpElement.innerHTML
               .split("%runtimeDetails%")
               .join(platform.description);
            helpElement.innerHTML = helpElement.innerHTML
               .split("%databaseInfo%")
               .join(databaseInfo);
   ...
   async onPostCashoutOkayClick() {
      var postCashoutElement =
         this.getTemplateByName("postCashout").elements[0];
   ...
   async onAccountButtonClick(buttonType, subType = null) {
      switch (buttonType) {
         ...
         case "create_account":
            if (subType == "login") {
               var element =
                  this.getTemplateByName("accountCreate").elements[0];
               this.show(element);
            ...
            this.cypherpoker
               .createAccount(typeSplit[0], password, typeSplit[1])
               .then((newAccount) => {
                  this.hide(element);
                  element = this.getTemplateByName("help").elements[0];
                  if (typeSplit[0] == "bitcoin") {
                     if (typeSplit[1] == "test3") {
                        var helpElement = element.querySelector(
                           "#new_account_btc_test3"
                        );
                     } else {
                  ...
                  helpElement.innerHTML = helpElement.innerHTML
                     .split("%address%")
                     .join(newAccount.address);
                  helpElement.innerHTML = helpElement.innerHTML
                     .split("%depositfee%")
                     .join(newAccount.fees.deposit);
                  this.show(helpElement);
                  this.showDialog();
                  ...
                  var manageElement =
                     this.getTemplateByName("accountManage").elements[0];
                  var currentAccounts =
                     element.querySelector("#currentAccounts");
                  currentAccounts.appendChild(newOptionElement);
                  currentAccounts.value = optionValue; //set new item as current selection
                  element.querySelector("#accountPassword").value =
                     newAccount.password; //set password for account
               })
               .catch((error) => {
         ...
         case "use_account":
            ...
            var selectedOption =
               accountsList.options[accountsList.selectedIndex];
            ...
            if (this.cypherpoker.p2pConnected == false) {
               ...
               msg +=
                  "You won't be able to create or join tables until you're connected.";
               this.showDialog(msg);
         ...
         case "cashout_account":
            ...
            var cashoutAddress =
               manageElement.querySelector("#cashoutAddress").value;
            var cashoutAmount =
               manageElement.querySelector("#cashoutAmount").value;
            var selectElement = element.querySelector("#currentAccounts");
            var selectedOption =
               selectElement.options[selectElement.selectedIndex];
            if (cashoutAmount.split(" ").join("") == "") {
               ...
               try {
                  ...
                  this.showDialog(
                     "Invalid cashout amount. Must be in satoshis."
                  );
                  this.hideDialog(3000);
                  return false;
               }
            ...
            this.selectedAccount
               .cashout(cashoutAmount, cashoutAddress, minerFee)
               .then((cashoutResult) => {
                  var satoshiAmount = this.selectedAccount.balance.toString(10);
                  var btcAmount = this.convertDenom(
                     satoshiAmount,
                  ...
                  if (this.selectedAccount.type == "bitcoin") {
                     if (this.selectedAccount.network == "test3") {
                        //testnet
                        test3Element.innerHTML = test3Element.innerHTML
                           .split("%txHash%")
                           .join(cashoutResult.txHash);
                        this.hide(btcElement);
                        this.show(test3Element);
                     } else {
                        //mainnet
                        btcElement.innerHTML = btcElement.innerHTML
                           .split("%txHash%")
                           .join(cashoutResult.txHash);
                     ...
                     if (this.selectedAccount.network == "test") {
                        //testnet
                        bchTestElement.innerHTML = bchTestElement.innerHTML
                           .split("%txHash%")
                           .join(cashoutResult.txHash);
                        ...
                        //mainnet
                        bchElement.innerHTML = bchElement.innerHTML
                           .split("%txHash%")
                           .join(cashoutResult.txHash);
         ...
         case "transfer_account":
            ...
            element = this.getTemplateByName("accountLogin").elements[0];
            var transferAccount =
               manageElement.querySelector("#transferAccount").value;
            var transferAmount =
               manageElement.querySelector("#transferAmount").value;
            var selectElement = element.querySelector("#currentAccounts");
            var selectedOption =
               selectElement.options[selectElement.selectedIndex];
            if (transferAmount.split(" ").join("") == "") {
               ...
               try {
                  ...
                  this.showDialog(
                     "Invalid transfer amount. Must be in satoshis."
                  );
                  this.hideDialog(3000);
                  return;
               }
      ...
      switch (buttonType) {
         case "create_game":
            if (this.cypherpoker.p2pConnected == false) {
               var msg = "Peer-to-peer networking is not available.<br/>";
               msg +=
                  "You can't create or advertise a table until you're connected.";
               this.showDialog(msg);
               this.hideDialog(6000);
               return;
            }
            ...
            var smallBlindAmount =
               createGameElement.querySelector("#smallBlindAmount").value;
            var inactivityTimeout = Math.round(
               createGameElement.querySelector("#inactivityTimeoutAmount").value
            );
            //dev skip validation
            // var validationError =
            ...
            this.cypherpoker.addEventListener(
               "tableleave",
               this.onPlayerLeaveTable,
               this
            );
            this.cypherpoker.removeEventListener(
               "tableready",
               this.onTableReady
            );
            this.cypherpoker.addEventListener(
            ...
            var newTable = this.cypherpoker.createTable(
               tableName,
               numPlayers,
               tableInfo
            );
   ...
   validateTableCreateForm(createGameElement) {
      ...
      var smallBlindAmount =
         createGameElement.querySelector("#smallBlindAmount").value;
      var inactivityTimeout = createGameElement.querySelector(
         "#inactivityTimeoutAmount"
      ).value;
   ...
   onPlayerJoinTable(event) {
      try {
         ...
         var joinedTables = this.cypherpoker.getJoinedTables(
            tableName,
            tableID,
            ownerPID
         );
   ...
   onJoinTableButtonClick(event) {
      ...
      if (alias.trim() == "") {
         ui.showDialog(
            "You must enter a player alias (name) before attempting to join a table."
         );
         ui.hideDialog(4000);
         return;
      }
      ...
      //    ui.showDialog(
      //       "Insufficient account balance for buy-in.<br/>Minimum account balance " +
      //          buyInAmount +
      //          " required."
      //    );
      //    ui.hideDialog(4000);
      ...
      ui.cypherpoker.addEventListener(
         "tablejoinrequest",
         ui.onPlayerJoinTable,
         ui
      );
      ...
      ui.cypherpoker.joinTable(table).catch((err) => {
         ...
         ui.hideDialog(4000);
         console.log(
            "Table join request has timed out.***************************"
         );
   ...
   onTableReady(event) {
      ...
      var game = this.cypherpoker
         .createGame(event.table, this.selectedAccount, playerInfo)
         .start();
      try {
         game.addEventListener("gamerestart", this.onRestartGame, this);
         game.addEventListener("gamekill", this.onKillGame, this);
         game.contract.addEventListener(
            "timeoutstart",
            this.onStartContractTimeout,
            this
         );
         game.contract.addEventListener(
            "timeout",
   ...
   /**
    * Event listener invoked when the user interface's "call" button is clicked.
    *
   ...
   onCallButtonClick(event) {
      ...
      let callbuttonText = game.DOMElement.querySelector(
         ui.gameUISelectors.callButton
      ).innerText;

      let betAmount = 0;
      //get minimum bet amount
      let prevBetTotal = game.getPlayer(game.ownPID).totalBet.value;
      console.log("prevBetTotal: " + prevBetTotal);
      if (callbuttonText !== "Check") {
         betAmount = parseInt(callbuttonText.split(" ")[1]);
      } else {
         betAmount = 0;
      }

      console.log("&*&*&*&*&* call this amt: " + betAmount);
      betAmount = betAmount + prevBetTotal;
      try {
         var amount = bigInt(betAmount);
         if (amount.lesser(0)) {
            this.showDialog("call amount must be positive.");
            this.hideDialog(4000);
            return;
         }
         ...
         ui.enable(
            game.DOMElement.querySelector(ui.gameUISelectors.foldButton)
         );
      }
      game.DOMElement.querySelector(ui.gameUISelectors.potAmount).innerHTML =
         game.pot.toString(10) + " satoshis";
   ...
   onBetButtonClick(event) {
      ...
      var ui = event.target.ui;
      ui.disable(event.target);
      ui.disable(game.DOMElement.querySelector(ui.gameUISelectors.callButton));
      ui.disable(game.DOMElement.querySelector(ui.gameUISelectors.foldButton));
      var betAmount = game.DOMElement.querySelector(
         ui.gameUISelectors.betAmount
      );
      console.log("onBetButtonClick");
      console.log("&*&*&*&*&* bet this amt: " + betAmount.value);
      try {
   ...
   onRestartGame(event) {
      try {
         event.game.contract.addEventListener(
            "timeoutstart",
            this.onStartContractTimeout,
            this
         );
    ...
    */
   onKillGame(event) {
      var timeoutElement = event.game.DOMElement.querySelector(
         ui.gameUISelectors.timeoutAmount
      );
      this.stopTimeoutTimer(timeoutElement);
      try {
         event.game.removeEventListener(
            "gamerestart",
            this.onRestartGame,
            this
         );
         event.game.removeEventListener("gamekill", this.onKillGame, this);
         event.game.contract.removeEventListener(
            "gamerestart",
            this.onRestartGame,
            this
         );
   ...
   onStartContractTimeout(event) {
      var timeoutElement = event.contract.game.DOMElement.querySelector(
         ui.gameUISelectors.timeoutAmount
      );
   ...
   onContractTimeout(event) {
      var timeoutNotification =
         "The following player(s) have timed out:<br/><br/>";
      for (var count = 0; count < event.penalized.length; count++) {
         var timedOutPlayer = event.contract.getPlayer(
            event.penalized[count].privateID
         );
         event.contract.game.debug(
            "Player has timed out: " + timedOutPlayer.account.address + "<br/>",
            "err"
         );
         timeoutNotification +=
            timedOutPlayer.info.alias +
            " (" +
            timedOutPlayer.account.address +
            ")<br/>";
         timeoutNotification +=
            " Private ID: " + event.penalized[count].privateID + "<br/>";
      }
      ...
      this.stopTimeoutTimer(timeoutElement);
      try {
         event.contract.removeEventListener(
            "gamerestart",
            this.onRestartGame,
            this
         );
      ...
      var nextDealerPID = event.contract.game.getNextPlayer(
         event.contract.game.getDealer().privateID
      ).privateID;
      if (nextDealerPID == event.contract.game.ownPID) {
         //if we want to auto-restart we should do that below
         this.enable(
            event.contract.game.DOMElement.querySelector(
               this.gameUISelectors.newHandButton
            )
         );
      } else {
   ...
   onNewTableAnnouncement(event) {
      ...
      metaTags.numPlayers = String(
         tableData.requiredPID.length + tableData.joinedPID.length
      );
      ...
      metaTags.timeout = tableData.tableInfo.timeout;
      var joinTableButton = this.cloneHTMLTemplate(
         templateInfo,
         containerElement,
         false,
         metaTags
      ...
      this.cypherpoker.announcedTables[0].tableInfo.buttonElement =
         joinTableButton;
   ...
   onNewGame(event) {
      ...
      var newGameElement = this.cloneHTMLTemplate(
         gameTemplate,
         target,
         gameTemplate.hidden,
         metaData
   ...
   onGameDealMsg(event) {
      let game = event.game;
      let temper = game.getDealer();
      if (game.ownPID == temper.privateID) {
         game.DOMElement.querySelector(
            this.gameUISelectors.dealerPuck
         ).textContent = "true";
         document.getElementById("dealerButton").style.display = "block";
         document.getElementById("villainDealerButton").style.display = "none";
      } else {
         game.DOMElement.querySelector(
            this.gameUISelectors.dealerPuck
         ).textContent = "false";
         document.getElementById("dealerButton").style.display = "none";
         document.getElementById("villainDealerButton").style.display = "block";
      }
   }
   ...
   onCardDeal(event) {
      var game = event.game;
      var targetElement = game.DOMElement.querySelector(
         ui.gameUISelectors.privateCards
      );
      if (event.private == false) {
         targetElement = game.DOMElement.querySelector(
            ui.gameUISelectors.publicCards
         );
      }
      ...
      game.DOMElement.querySelector(this.gameUISelectors.potAmount).innerHTML =
         game.pot.toString() + " satoshis";

      if (game.canBet) {
         console.log("i can bet*****************************");
         this.updateMinimumBet(game);

         if (game.cardDecks.public.length > 0) {
            game.DOMElement.querySelector(
               this.gameUISelectors.betButton
            ).innerHTML = "Bet " + game.table.tableInfo.bigBlind;

            game.DOMElement.querySelector(
               this.gameUISelectors.betAmount
            ).value = game.table.tableInfo.bigBlind;
         }

         this.enable(
            game.DOMElement.querySelector(this.gameUISelectors.betButton)
         );
         this.enable(
            game.DOMElement.querySelector(this.gameUISelectors.foldButton)
         );
         game.DOMElement.querySelector(
            this.gameUISelectors.callButton
         ).innerText = "Check";
         this.enable(
            game.DOMElement.querySelector(this.gameUISelectors.callButton)
         );
      }
   ...
   onBetPlaced(event) {
      ...
      this.updateTotalBet(game);
      if (game.canBet) {
         let ownTotalBet = game.getPlayer(game.ownPID).totalBet.value;
         //   console.log("ownTotalBet", ownTotalBet);
         //!!!!action is to us bet we dont' know if we can bet.
         this.updateMinimumBet(game);
         console.log(
            "%%%%%%%%%%%%%%%%%%%%%% min bet ",

            game.minimumBet.value,
            "own total bet",
            ownTotalBet,
            "largest bet",
            game.largestBet.value
            // this.players[0].totalBet.value,
            // this.players[1].totalBet.value
         );
         let callingBet =
            game.largestBet.value -
         ...
         let facingBet = document.getElementById("villainBetsValue");

         if (game.largestBet.value > 0) {
            console.log("cant check");
            game.DOMElement.querySelector(
               this.gameUISelectors.callButton
            ).innerText = "Call" + " " + callingBet;
         }
         if (game.largestBet.value == 0) {
         ...
         //if not facing a bet or raise we can check for betting else we can check for raising or re-raising
         //get previous bet
         let canRaise = true;
         ...
         if (canRaise) {
            game.DOMElement.querySelector(
               this.gameUISelectors.betButton
            ).innerText = "Raise to" + " " + game.largestBet.value * 2;
            game.DOMElement.querySelector(
               this.gameUISelectors.betAmount
            ).value = game.largestBet.value * 2;
         }
         if (game.largestBet.value == 0) {
            game.DOMElement.querySelector(
               this.gameUISelectors.betButton
            ).innerText = "Bet" + " " + game.table.tableInfo.bigBlind;
            game.DOMElement.querySelector(
               this.gameUISelectors.betAmount
            ).value = game.table.tableInfo.bigBlind;
         }
         //increase the number in the bet box
   ...
   async onGameEnd(event) {
      ...
      var nextDealerPID = game.getNextPlayer(
         game.getDealer().privateID
      ).privateID;
      if (nextDealerPID == game.ownPID) {
         //if we want to auto-restart we should do that below
         this.enable(
            game.DOMElement.querySelector(this.gameUISelectors.newHandButton)
         );
      } else {
   ...
   onGameScored(event) {
      ...
      try {
         ...
         for (var count = 0; count < winningHands.length; count++) {
            ...
            var targetElement =
               event.game.DOMElement.querySelector("#handHistory");
            var metaTags = new Object();
            ...
            var newHistoryElement = this.cloneHTMLTemplate(
               templateInfo,
               targetElement,
               false,
               metaTags
            );
            var cardContainerElement = newHistoryElement.querySelector(
               "#handHistoryItemCards"
            );
   ...
   updateTotalBet(game) {
      ...
      var totalBetDiv = game.DOMElement.querySelector(
         this.gameUISelectors.totalBet
      );
      var balanceDiv = game.DOMElement.querySelector(
         this.gameUISelectors.balance
      );
      balance.innerHTML =
         String(game.getPlayer(game.ownPID).balance) + " satoshis";
   ...
   updateMinimumBet(gameRef) {
      var betAmountElement = gameRef.DOMElement.querySelector(
         this.gameUISelectors.betAmount
      );
   ...
   resetGameUI(gameRef, delay = 0, context = null) {
      ...
      context.debug(
         "resetGameUI(" + gameRef + ", " + delay + ", " + context + ")"
      );
      if (delay > 0) {
         setTimeout(context.resetGameUI, delay, gameRef, 0, context);
      }
      context.hideDialog.call(context, 3000);
      gameRef.DOMElement.querySelector(
         context.gameUISelectors.publicCards
      ).innerHTML = "";
      gameRef.DOMElement.querySelector(
         context.gameUISelectors.privateCards
      ).innerHTML = "";
      gameRef.DOMElement.querySelector(context.gameUISelectors.totalBet).value =
         "0";
   }
   ...
   updateAccountsUI(context = null) {
      ...
      if (context.cypherpoker.accounts.length == 0) {
         ...
         var loginElement =
            context.getTemplateByName("accountLogin").elements[0];
         var manageElement =
            context.getTemplateByName("accountManage").elements[0];
         var accountsList = loginElement.querySelector("#currentAccounts");
         var minerFeeInput = manageElement.querySelector("#minerFee");
         for (
            var count = 0;
            count < context.cypherpoker.accounts.length;
            count++
         ) {
         ...
         loginElement.querySelector("#accountBalance").innerHTML =
            "updating...";
         loginElement.querySelector("#accountPassword").value =
            account.password;
         account
            .update()
            .then((done) => {
               var satoshiAmount = account.balance.toString(10);
               var btcAmount = context.convertDenom(
                  satoshiAmount,
                  "satoshi",
                  "bitcoin"
               );
   ...
   removeAllJoinTableButtons() {
      for (
         var count = 0;
         count < this.cypherpoker.announcedTables.length;
         count++
      ) {
      ...
      switch (fromDenom) {
         case "satoshi":
            if (toDenom == "bitcoin") {
               ...
               throw new Error(
                  'Unrecognized target denomination "' + toDenom + '"'
               );
            }
            break;
         case "bitcoin":
         ...
         default:
            throw new Error(
               'Unrecognized source denomination "' + fromDenom + '"'
            );
            break;
      }
      return amount;
